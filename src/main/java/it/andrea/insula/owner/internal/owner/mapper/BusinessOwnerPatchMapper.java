...
import it.andrea.insula.owner.internal.owner.dto.request.BusinessOwnerPatchDto;
...
@Component
public class BusinessOwnerPatchMapper implements BiFunction<BusinessOwnerPatchDto, BusinessOwner, BusinessOwner> {

    @Override
    public BusinessOwner apply(BusinessOwnerPatchDto dto, BusinessOwner owner) {
        if (dto.email() != null) owner.setEmail(dto.email());
        if (dto.phoneNumber() != null) owner.setPhoneNumber(dto.phoneNumber());
        if (dto.companyName() != null) owner.setCompanyName(dto.companyName());
        if (dto.fiscalCode() != null) owner.setFiscalCode(dto.fiscalCode());
        if (dto.vatNumber() != null) owner.setVatNumber(dto.vatNumber());
        if (dto.sdiCode() != null) owner.setSdiCode(dto.sdiCode());
        if (dto.pecEmail() != null) owner.setPecEmail(dto.pecEmail());
